...
void ExpectAnalyticEngine(const numeraire::core::IPricer& p) {
    EXPECT_EQ(p.EngineKind(), numeraire::PricingEngineType::kAnalytic);
}
...
TEST(IPricerTest, StubPricesIntrinsicCall) {
    ...
    const numeraire::core::PricingResult r = pricer.Price(opt, mkt);
    ASSERT_TRUE(r.Npv().has_value());
    EXPECT_DOUBLE_EQ(*r.Npv(), 5.0);
}
...
TEST(IPricerTest, PolymorphicDispatchThroughInterface) {
    ...
    ExpectAnalyticEngine(ref);
    const numeraire::core::PricingResult priced = ref.Price(opt, mkt);
    ASSERT_TRUE(priced.Npv().has_value());
    EXPECT_DOUBLE_EQ(*priced.Npv(), 0.0);
}
