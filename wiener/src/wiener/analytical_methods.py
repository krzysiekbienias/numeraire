...
class EuropeanPlainVanillaOption(AnalyticalPricingEnginesInterface):
    ...
    def prepare_priceable_dictionary(self,**kwargs) -> dict:
        ...
        if self._trade_id is not None:
           ...
           market_data_dict['discount_factor']=kwargs['discount_factor'] if 'discount_factor' in kwargs else self._market_environment.extract_discount_factors(maturity_date=market_data_dict['trade_maturity']) 
    ...
    def run_analytical_pricer(self):
        ...
        if self.pricable_dict['payoff']=="call":
            return self.pricable_dict["underlier_price"]*norm.cdf(d1,0,1)-self.pricable_dict["strike"]*self.pricable_dict["discount_factor"]*norm.cdf(d2,0,1)
        else:
            return self.pricable_dict["strike"]*self.pricable_dict["discount_factor"]*norm.cdf(-d2,0,1)-self.pricable_dict["underlier_price"]*norm.cdf(-d1,0,1)
