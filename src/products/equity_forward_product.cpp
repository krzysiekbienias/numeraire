...
std::string_view EquityForwardProduct::UnderlyingId() const {
    return underlying_id_;
}

OptionType EquityForwardProduct::OptionKind() const {
    return OptionType::kCall;
}

ExerciseStyle EquityForwardProduct::Exercise() const {
...
const schedule::Date& EquityForwardProduct::ExpiryDate() const {
    return expiry_date_;
}
...
bool EquityForwardProduct::UsesImpliedVolatility() const {
    return false;
}

bool EquityForwardProduct::HasCalendarMaturity() const {
    return true;
}
