...
[[nodiscard]] std::optional<VolSurfaceEodRead> ReadVolSurfaceEodFromDb(
...
bool HasVolSurfaceEod(const std::string& database_file_path,
                      const std::string_view underlying_id,
                      const std::string_view as_of_iso_yyyy_mm_dd,
                      const std::string_view surface_kind) {
    try {
        ...
        SQLite::Statement hdr(
                db,
                "SELECT 1 FROM vol_surface_eod WHERE underlying_id = ? AND as_of = ? AND surface_kind = ? LIMIT 1");
        hdr.bind(1, std::string(underlying_id));
        hdr.bind(2, std::string(as_of_iso_yyyy_mm_dd));
        hdr.bind(3, std::string(surface_kind));
        return hdr.executeStep();
    } catch (SQLite::Exception const& e) {
        throw PersistenceError(std::string{"HasVolSurfaceEod: "} + e.what());
    }
}

std::optional<VolSurfaceEodRead> TryLoadVolSurfaceEod(const std::string& database_file_path,
                                                        const std::string_view underlying_id,
                                                        const std::string_view as_of_iso_yyyy_mm_dd,
                                                        const std::string_view surface_kind) {
    try {
        return ReadVolSurfaceEodFromDb(database_file_path, underlying_id, as_of_iso_yyyy_mm_dd, surface_kind, false);
    } catch (SQLite::Exception const& e) {
        throw PersistenceError(std::string{"TryLoadVolSurfaceEod: "} + e.what());
    }
}
...
VolSurfaceEodRead LoadVolSurfaceEod(const std::string& database_file_path,
                                      ...
                                      const std::string_view surface_kind) {
    try {
        std::optional<VolSurfaceEodRead> loaded =
                ReadVolSurfaceEodFromDb(database_file_path, underlying_id, as_of_iso_yyyy_mm_dd, surface_kind, true);
        return *loaded;
    } catch (SQLite::Exception const& e) {
