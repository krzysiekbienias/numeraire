...
std::optional<double> LookupEquityDailyClose(const std::string& database_file_path,
    ...
    try {
        ...
        SQLite::Statement st(
                db,
                "SELECT close FROM equity_daily_eod WHERE UPPER(ticker) = UPPER(?) AND as_of = ? AND "
        ...
        throw PersistenceError(std::string{"equity_daily_eod lookup: "} + e.what());
    }
}
