...
/// Index `v2/aggs` daily bars: omit `adjusted` query param (not used like equity splits).
[[nodiscard]] std::string BuildIndexAggsUrl(const std::string& base,
...
[[nodiscard]] int UpsertIndexBars(SQLite::Database& db,
                                  const std::string& ticker,
                                  const nlohmann::json& results,
                                  const int adjusted_flag,
...
[[nodiscard]] int IngestIndexTickerRange(SQLite::Database& db,
    ...
    std::string url = BuildIndexAggsUrl(base_url, ticker, from_iso, to_iso);
    int total_upserted = 0;
    const std::string ingested_at = IsoUtcNow();
    const int adjusted_flag = 1;
    ...
    for (;;) {
        ...
        if (!j.contains("results") || !j["results"].is_array() || j["results"].empty()) {
            Logger::NumWarn("Index aggs: no daily results for {} in range {}..{}.", ticker, from_iso, to_iso);
            break;
        }
        const int chunk = UpsertIndexBars(db, ticker, j["results"], adjusted_flag, source_label, ingested_at);
        total_upserted += chunk;
        Logger::NumInfo("Index aggs: upserted {} row(s) for {} (chunk running total {}).",
                        chunk,
                        ticker,
                        total_upserted);
        if (next_url.empty()) {
    ...
    Logger::NumInfo("Index aggs: done {} total bar row(s) for {}.", total_upserted, ticker);
    return 0;
}
...
void PrintIndexFetchUsageLines() {
    Logger::NumError(
            "  dev_main --fetch-index-eod-daily --from YYYY-MM-DD --to YYYY-MM-DD [--ticker I:SPX ...]\n"
...
int TryRunPolygonIndexDailyEodFetch(const int argc, char** argv, const numeraire::utils::Config& cfg) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--fetch-index-eod-daily") == 0) {
            fetch_mode = true;
    ...
    if (from_iso.empty() || to_iso.empty()) {
        Logger::NumError("--fetch-index-eod-daily requires --from and --to.");
    ...
    try {
        ...
        for (const std::string& t : tickers) {
            if (IngestIndexTickerRange(db, base, key, t, from_iso, to_iso, throttle_sec) != 0) {
                return 1;
            }
        }
    ...
    Logger::NumInfo("--fetch-index-eod-daily finished for {} ticker(s).", tickers.size());
    return 0;
}
