...
- **`vol_surface_eod` / `vol_surface_point_eod`** — official EOD implied-vol surface per `(underlying_id, as_of, surface_kind)` plus sparse `(expiration_date, strike, call|put)` points. Built by `dev_main --build-vol-surface-eod` (quant inversion in `numeraire_quant`, SQLite I/O in `numeraire_database`). Interpolation into `IMarketData` is still open.

```bash
./build/dev_main --build-vol-surface-eod --underlying NDX --from 2026-05-01 --to 2026-05-22
# spot: index_daily_eod (default I:NDX for NDX); rate/div: NUMERAIRE_DEV_RATE / NUMERAIRE_DEV_DIV_YIELD
```

**Visualize** (Jupyter): [`viz/README.md`](../viz/README.md) — `pip install -e "viz/[notebook]"`, then open [`viz/notebooks/vol_surface_eod.ipynb`](../viz/notebooks/vol_surface_eod.ipynb).
